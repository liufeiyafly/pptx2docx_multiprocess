...
def ppt2pptx(filename):
    ...
    # filename=r'C:\Users\liufe\Desktop\1.pptx'
    ...
    for i in range(num):
        ...
        for j in body_shapes:
            if str(type(j)) == auto_object or str(type(j)) == place_object:  # 图形或者文本框是有 .text属性的，可以直接访问
                # print(j.text.encode('gbk','ignore').decode('gbk','ignore'))
                s1 = j.text.encode('gbk', 'ignore').decode('gbk', 'ignore')  # 我靠，用这个代码解决不能打印和写入文件'gbk'特殊编码的问题了！！！
                # print(s1)
